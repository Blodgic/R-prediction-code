...
#SourceFire <- read.csv(file.choose(), sep = ",")
SourceFire <- read.csv("~/Downloads/community.rules.txt/communityrules.csv", sep = ",")
SourceFire <- as.data.frame(SourceFire)
#SourceFire$simple_category <- gsub("\\-.*","", SourceFire$category)
...
#sigs that are not deleted
SourceFire_non_deleted <- SourceFire[ which(SourceFire$category!='deleted.rules'),]

SourceFire_deleted <- SourceFire[ which(SourceFire$category=='deleted.rules'),]
nrow(SourceFire_deleted)

#new sigs from last changed 
SourceFire_non_deleted_new <- SourceFire_non_deleted[ which(SourceFire_non_deleted$last_changed > '1430000000'),]
#create a blank column in SourceFire_non_deleted_new
SourceFire_non_deleted_new$BLOOM_Category_NEW <- ""
SourceFire_non_deleted_new_categorized <- SourceFire_non_deleted_new

View(SourceFire_non_deleted_new_categorized)
...
View(SourceFire_non_deleted[grep("FILE*", SourceFire_non_deleted$msg), ])
attempted_user <- SourceFire_non_deleted[grep("attempted-user", SourceFire_non_deleted$classtype), ]
table(attempted_user$sip)
View(SourceFire_non_deleted)
View(SourceFire_non_deleted[grep("<>", SourceFire_non_deleted$diroperator), ])
...
#plot of SourceFire signatures with classtype
Class_type <- transform(Class_type, Var1 = reorder(Var1, Freq))
ggplot(data = Class_type, aes(x = Var1, y = Freq, fill=Var1)) +  geom_bar(stat="identity") + geom_text(aes(label=Freq), size=5) + ggtitle("SourceFire Class Types") +
  coord_flip()


#plot of Sourcefire signatures categories

SourceFire_category_gr8_300 <- transform(SourceFire_category_gr8_300, Var1 = reorder(Var1, Freq))
ggplot(data = SourceFire_category_gr8_300, aes(x = Var1, y = Freq, fill=Var1)) + geom_bar(stat="identity") + geom_text(aes(label=Freq), size=5) + ggtitle("SourceFire_category_gr8_300") +
  coord_flip()



SourceFire_category_300_100 <- transform(SourceFire_category_300_100, Var1 = reorder(Var1, Freq))
ggplot(data = SourceFire_category_300_100, aes(x = Var1, y = Freq, fill=Var1)) + geom_bar(stat="identity") + geom_text(aes(label=Freq), size=5) + ggtitle("SourceFire_category_300_100") +
  coord_flip()

SourceFire_category_less_100 <- transform(SourceFire_category_less_100, Var1 = reorder(Var1, Freq))
ggplot(data = SourceFire_category_less_100, aes(x = Var1, y = Freq, fill=Var1)) + geom_bar(stat="identity") + geom_text(aes(label=Freq), size=5) + ggtitle("SourceFire_category_less_100") +
  coord_flip()

#plot of SourceFire signatures with CVE per year
ggplot(data = CVE_year, aes(x = Var1, y = Freq)) + geom_bar(stat="identity") + ggtitle("SourceFire_CVE_per_Year" )+ geom_text(aes(label=Freq), position=position_dodge(width=0.9), vjust=-0.25)
...
ggplot(data = CVE_year_et.pro, aes(x = Var1, y = Freq)) + ggtitle("ET PRO Distribution per year") + geom_bar(stat="identity") + geom_text(aes(label=Freq), position=position_dodge(width=0.9), vjust=-0.25)
...
#write.csv(SourceFire_non_deleted, "Downloads/SourceFire_non_deleted_categorized.csv")
